...
class CustomDataset(Dataset):
    def __init__(self, df, cfg, aug, mode="train"):
        ...
        # do not use records wo input data
        self.df = self.df[self.df.target_path.isin(self.sat_meta_dict.keys())]
    ...
    def get_sat_img(self, source, band, lat, lon, source_date):
        ...
        if self.cfg.normalization == "minmax":
            ...
            if x.max() > x.min():
                ...
                x = np.zeros((self.cfg.image_size, self.cfg.image_size))
        elif self.cfg.normalization == "band":
            x = (x - band_min) / (band_max - band_min)
    
        x = cv2.resize(x, dsize=(self.cfg.image_size, self.cfg.image_size))
        ...
        for source in self.cfg.sources:
            for source_rank in range(1, self.cfg.max_rank + 1):
                ...
                for band in self.cfg.BAND_LIMITS[source].keys():
                    if len(row) == 0:
                        ch = np.zeros((self.cfg.image_size, self.cfg.image_size))
                    else:
    ...
    def __getitem__(self, idx):
        ...
        if target_path in self.sat_meta_dict:
            ...
            if self.mode == "train" and self.cfg.source_date_dropout > 0.0 and len(df) > 2:
                if np.random.rand() < self.cfg.source_date_dropout:
                    n_start = int(len(df) * self.cfg.source_date_keep_ratio)
                    n = np.random.randint(n_start, len(df))  # sample satellite dates
                    df = df.sample(n=n)
